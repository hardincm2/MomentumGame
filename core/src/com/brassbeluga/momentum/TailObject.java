...
public class TailObject extends GameObject {
	...
	public TailObject(Spider parent, float x, float y, float xOff, float yOff, TextureRegion texture) {
		super(x, y, texture);
		texOff.set((texOff.x / texture.getRegionWidth()) * rect.width, (texOff.y / texture.getRegionHeight()) * rect.height);
		curlOff.set((curlOff.x / Assets.catTailCurl.getRegionWidth()) * rect.width, (curlOff.y / Assets.catTailCurl.getRegionHeight()) * rect.width);
		...
		if (parent != null) {
			...
			if (parent.peg == null) {
				batch.draw(region, parent.x + offset.x - texOff.x, parent.y + offset.y - texOff.y, 0, 0, 
						rect.width, rect.height, 1.0f, 1.0f, parent.angle);
			}else{
